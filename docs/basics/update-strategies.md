...
!!!warning "A note on the current image tag"
    ...
    semver. Otherwise no comparison can happen by the updater. See discussion at [#270](https://github.com/nholuongut/argocd-image-updater/issues/270) for more details.
...
like `a5fb3d3` or `f7bb2e3`, but not `latest` or `main`.
...
argocd-image-updater.argoproj.io/image-list: myimage=some/image
argocd-image-updater.argoproj.io/myimage.update-strategy: latest
argocd-image-updater.argoproj.io/myimage.ignore-tags: latest, main
```
...
This would allow for considering all tags found but `latest` and `main`. You
